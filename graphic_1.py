...
class Graphic_builder:
    ...
    def extract_data(self):
        self.c.execute(f"""SELECT * FROM storage_of_results WHERE test_name = '{self.test_name}'""")
    ...
    def build_gpaph(self):
        ...
        plt.savefig(f"{self.test_name}.png", dpi = 400, bbox_inches='tight')
        plt.show()
   
    def run(self):
        self.extract_data()
        d.remove_duplicates_n_sort_by_date()
        d.build_gpaph()

    
d = Graphic_builder('Креатинин')
d.run()
